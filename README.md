...
Just put `hbase-protocol-*.jar` in
`/opt/cloudera/parcels/CDH/lib/spark/assembly/lib/` and bunced
`/opt/cloudera/parcels/CDH/lib/spark/assembly/lib`

## Examples

SparkOnHBase comes with a number of examples.  Here is the CLI
commands to try them out.  These work with a table named 't1' and
column fmaily 'c'
