...
class HBaseContext(@transient sc: SparkContext,
  ...
  def javaBulkPut[T](javaDdd: JavaRDD[T], 
    ...
    bulkPut(javaDdd.rdd, tableName, (t:T) => f.apply(t), autoFlush)
  }
  ...
  def javaBulkCheckAndPut[T](javaRdd: JavaRDD[T], 
    ...
    bulkCheckAndPut(javaRdd.rdd, tableName, (t:T) => f.apply(t), autoFlush)
  }
  ...
  def javaBulkIncrement[T](javaRdd: JavaRDD[T], tableName: String,
    ...
    bulkIncrement(javaRdd.rdd, tableName, (t:T) => f.apply(t), batchSize)
  }
  ...
  def javaBulkDelete[T](javaRdd: JavaRDD[T], tableName: String,
    ...
    bulkDelete(javaRdd.rdd, tableName, (t:T) => f.apply(t), batchSize)
  }
   ...
   * @batchSize       The number of delete to batch before sending to HBase
   */
  def bulkCheckDelete[T](rdd: RDD[T], tableName: String, f: (T) => (Array[Byte], Array[Byte], Array[Byte], Array[Byte], Delete), autoFlush: Boolean) {
    rdd.foreachPartition(
      it => hbaseForeachPartition[T](
        broadcastedConf,
        it,
  ...
  /**
   * A simple abstraction over the HBaseContext.foreachRDD method.
   *
   * It allow addition support for a user to take a DStream and
   * generate Increments and send them to HBase.
   *
  ...
  def streamBulkCheckAndDelete[T](dstream: DStream[T], 
      ...
      autoFlush: Boolean) {
    dstream.foreach((rdd, time) => {
      bulkCheckDelete(rdd, tableName, f, autoFlush)
    })
  }
  ...
  def javaBulkGet[T, U](tableName: String,
    ...
    JavaRDD.fromRDD(bulkGet(tableName,
        ...
        javaRdd.rdd,
        (t:T) => makeGet(t),
        (r:Result) => {convertResult(r)}))(fakeClassTag[U])
  }
